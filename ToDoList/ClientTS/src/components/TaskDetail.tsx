...
const TaskDetail: React.FC<TaskDetailProps> = ({ task, onUpdate, onClose }) => {
  ...
  const [responseLoading, setResponseLoading] = useState<{ [key: number]: boolean }>({});
  const [responseError, setResponseError] = useState<{ [key: number]: string | null }>({});
  ...
  const handleResponseChange = (messageId: number, value: string) => {
                ...
                {messages.map((message, index) => (
                  ...
                  >
                    <div style={{ fontSize: '12px', color: '#888', marginBottom: '4px' }}>
                      {message.userName} • {message.createdAt && !isNaN(new Date(message.createdAt as string).getTime())
                        ? format(new Date(message.createdAt as string), 'MMM dd, yyyy HH:mm')
                        : 'Unknown date'}
                    </div>
